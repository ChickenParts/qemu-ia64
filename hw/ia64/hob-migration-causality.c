/*
 * IA-64 PEI firmware-volume HOB migration causality probe
 *
 * SPDX-License-Identifier: GPL-2.0-or-later
 *
 * Disabled unless QEMU_IA64_PEI_FV_HOB_RESTORE is set.  This is a diagnostic
 * experiment, not a production firmware workaround.  It copies only existing,
 * structurally validated EFI_HOB_TYPE_FV records from a valid temporary HOB
 * list into a valid permanent HOB list that is missing those records.
 */
...
static struct Ia64HobCausalityState ia64_hob_causality;

static bool ia64_hob_causality_enabled(void)
{
                           ...
                           struct Ia64HobListInfo *info)
{
    ...
    if (offset > window_size || window_size - offset < IA64_HOB_HANDOFF_SIZE ||
        lduw_le_p(window + offset) != IA64_HOB_HANDOFF ||
...
static bool ia64_hob_write_le64(uint64_t address, uint64_t value)
{
    uint8_t encoded[8];

    stq_le_p(encoded, value);
    return ia64_hob_write(address, encoded, sizeof(encoded));
}
                                     ...
                                     const struct Ia64HobListInfo *target)
{
    ...
    added_bytes = (uint64_t)missing * sizeof(struct Ia64HobFvRecord::raw);
    if (uadd64_overflow(insertion, added_bytes, &new_end) ||
        new_end > UINT64_MAX - IA64_HOB_HEADER_SIZE ||
    ...
    if (target->free_bottom >= target->end_address) {
        if (uadd64_overflow(target->free_bottom, added_bytes,
                            &new_free_bottom) ||
            !ia64_hob_write_le64(target->address + 40, new_free_bottom)) {
            return false;
        }
    }
    if (!ia64_hob_write_le64(target->address + 48, new_end)) {
        return false;
    }
...
static void ia64_hob_causality_poll(void *opaque)
{
    struct Ia64HobCausalityState *state = opaque;
...
static void ia64_hob_causality_machine_done(Notifier *notifier, void *opaque)
{
    MachineState *machine = MACHINE(qdev_get_machine());

    if (!ia64_hob_causality_enabled() ||
        strcmp(object_get_typename(OBJECT(machine)), "ipf-machine") != 0) {
        return;
    }

    ia64_hob_causality.enabled = true;
    ia64_hob_causality.timer = timer_new_ns(QEMU_CLOCK_VIRTUAL,
                                            ia64_hob_causality_poll,
                                            &ia64_hob_causality);
    timer_mod(ia64_hob_causality.timer,
              qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) + IA64_HOB_POLL_NS);
    qemu_log_mask(LOG_GUEST_ERROR,
                  "IA64 FV_HOB_RESTORE diagnostic probe enabled\n");
}

static Notifier ia64_hob_causality_notifier = {
    .notify = ia64_hob_causality_machine_done,
};

static void ia64_hob_causality_register(void)
{
    qemu_add_machine_init_done_notifier(&ia64_hob_causality_notifier);
}

type_init(ia64_hob_causality_register)

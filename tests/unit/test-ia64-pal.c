...
static void test_vm_summary(void)
{
    ...
    g_assert_cmpuint(field(result.v1, 16, 0xffff), ==,
                     UINT16_MAX);
    g_assert_cmphex(result.v2, ==, 0);
    assert_return_action(result);
}
...
{
    assert_vm_info(IA64_PAL_CACHE_TYPE_INSTRUCTION);
    assert_vm_info(IA64_PAL_CACHE_TYPE_DATA);
}
...
static void test_freq_ratios(void)
{
    IA64PALResult result = ia64_pal_result_freq_ratios();
    uint64_t itc_hz;

    g_assert_cmpint(result.status, ==, IA64_PAL_STATUS_SUCCESS);
    g_assert_cmpuint(ratio_denominator(result.v0), ==, 1);
    g_assert_cmpuint(ratio_numerator(result.v0), ==, 3);
    g_assert_cmpuint(ratio_denominator(result.v1), ==, 1);
...
int main(int argc, char **argv)
{
    ...
    g_test_add_func("/ia64/pal/freq-base", test_freq_base);
    g_test_add_func("/ia64/pal/freq-ratios", test_freq_ratios);
    return g_test_run();
}

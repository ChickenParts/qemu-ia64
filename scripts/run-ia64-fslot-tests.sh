...
Environment overrides:
  IA64_FSLOT_LOGDIR   (default: scratch/ia64_logs/fslot)
  IA64_FSLOT_OUTDIR   (default: scratch/ia64_fslot_selftest)
...
run_timeout="${1:-12s}"
logdir="${IA64_FSLOT_LOGDIR:-scratch/ia64_logs/fslot}"
outdir="${IA64_FSLOT_OUTDIR:-scratch/ia64_fslot_selftest}"
...
src="scripts/ia64-fslot-selftest.S"
obj="$outdir/fslot-selftest.o"
bin="$outdir/fslot-selftest.elf"
...
if [[ $rc -ne 124 && $rc -ne 137 ]]; then
  echo "fslot test run failed with rc=$rc" >&2
  exit $rc
fi
...
echo "fslot test serial: $serial_log"
...
if grep -q "r8=0000000066736c66" "$qemu_log"; then
  echo "saw F-slot FAIL marker in $qemu_log" >&2
  grep "r8=0000000066736c66" "$qemu_log" | head -n 5 >&2 || true
  exit 1
fi

if ! grep -q "r8=0000000066736c70" "$qemu_log"; then
  echo "missing F-slot PASS marker in $qemu_log" >&2
  exit 1
fi

echo "fslot tests passed"

...
static IA64PALResult ia64_pal_result(int64_t status, uint64_t v0,
...
IA64PALResult ia64_pal_result_cache_info(uint64_t level, uint64_t type)
{
    ...
    const uint8_t line_size = 6; /* log2(64-byte line) */
    const uint8_t stride = 6;    /* log2(64-byte stride) */
    bool unified;
    uint32_t cache_size;
    uint64_t info1 = 0;
                                         ...
                                         unsigned int itr_count)
{
    ...
    return ia64_pal_result(IA64_PAL_STATUS_SUCCESS, info1, info2, 0);
}
                                      ...
                                      unsigned int entry_count)
{
    ...
    if (level >= IA64_PAL_TC_LEVELS ||
        (type != IA64_PAL_TC_TYPE_INSTRUCTION &&
         type != IA64_PAL_TC_TYPE_DATA) ||
        entry_count == 0 || entry_count > UINT8_MAX) {
        return ia64_pal_result(IA64_PAL_STATUS_EINVAL, 0, 0, 0);
    }

    /*
     * Each software TC is fully associative: one set, one way per entry,
     * and no unified instruction/data store.
     */
